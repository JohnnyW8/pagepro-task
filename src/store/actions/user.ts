...
export const getUserPosts = (id: number): AppThunk => async (
    dispatch: Dispatch
) => {
    dispatch(dispatchApiAction(`users/${id}/posts`, GET_USER_POSTS));
};

export const removeUserPost = (itemId: number): AppThunk => async (
    dispatch: Dispatch
) => {
    dispatch(
...
export const addPost = (params: any): AppThunk => async (
    dispatch: Dispatch
) => {
    dispatch(
